...
export default function HuntDetailPage() {
  ...
  // Récupérer les détails de la chasse
  useEffect(() => {
    if (id) fetchHunt();
  }, [id]);
  ...
  const fetchHunt = async () => {
    ...
    try {
      const response = await axios.get(`/api/admin/hunts/${id}/hunt`, {
  ...
  const handleParticipate = async () => {
    ...
    console.log('Hunt ID:', id);    // Vérifiez que l'ID de la chasse est correct

    try {
      const response = await axios.post(`/api/admin/hunts/${id}/participate`, {}, {
        headers: {
      ...
      // Si l'utilisateur participe déjà, afficher le message
      if (response.status === 200) {
        setParticipating(true);  // Mettre à jour l'état pour afficher que l'utilisateur participe déjà
      }
  ...
  return (
    <div className="p-8 min-h-screen bg-gradient-to-br from-[#6B3FA0] to-[#A14CA0]">
      ...
      <div className="mt-6">
        {participating ? (
          <p className="text-green-500">Vous participez déjà à cette chasse !</p>
        ) : (
          <button
